...
sub test_switch_manual_enum: Test(9) {
    ...
    is($r->opcodes()->[0]->code(), $SwitchManualEnum::OPCODE_CODE_STRVAL, 'Equals');
    is($r->opcodes()->[0]->body()->value(), 'foobar', 'Equals');

    is($r->opcodes()->[1]->code(), $SwitchManualEnum::OPCODE_CODE_INTVAL, 'Equals');
    is($r->opcodes()->[1]->body()->value(), 0x42, 'Equals');

    is($r->opcodes()->[2]->code(), $SwitchManualEnum::OPCODE_CODE_INTVAL, 'Equals');
    is($r->opcodes()->[2]->body()->value(), 0x37, 'Equals');

    is($r->opcodes()->[3]->code(), $SwitchManualEnum::OPCODE_CODE_STRVAL, 'Equals');
