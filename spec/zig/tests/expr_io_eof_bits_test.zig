// Autogenerated from KST: please remove this line if doing any edits by hand!

const _imp_std = @import("std");
const _imp_kaitai_struct = @import("kaitai_struct");
const _imp_expr_io_eof_bits = @import("../formats/expr_io_eof_bits.zig");

test "ExprIoEofBits" {
    const file = try _imp_std.fs.cwd().openFile("../../src/nav_parent_switch.bin", .{});
    defer file.close();
    var buffer: [8]u8 = undefined;
    var reader = file.reader(&buffer);
    const allocator = _imp_std.testing.allocator;
    var arena = _imp_std.heap.ArenaAllocator.init(allocator);
    defer arena.deinit();
    var _io = _imp_kaitai_struct.KaitaiStream.fromFileReader(&reader);
    const r = try _imp_expr_io_eof_bits.ExprIoEofBits.create(&arena, &_io, null, null);
    try _imp_std.testing.expectEqual(5167, r.foo);
    try _imp_std.testing.expectEqual(15, r.bar);
    try _imp_std.testing.expectEqual(null, r.baz);
    try _imp_std.testing.expectEqualSlices(u8, &[_]u8{  }, r.align);
    try _imp_std.testing.expectEqual(null, r.qux);
}
