...
BOOST_AUTO_TEST_CASE(test_switch_repeat_expr_invalid) {
    ...
    BOOST_CHECK_EQUAL(static_cast<switch_repeat_expr_invalid_t::one_t*>(r->body()->at(0))->first(), std::string("\x40\x40\x04\x37", 4));
    BOOST_CHECK(!r->body()->at(1)); // added manually
    BOOST_CHECK_EQUAL(r->_raw_body()->at(1), std::string("\x13\x00\x00\x08", 4));
    BOOST_CHECK_EQUAL(static_cast<switch_repeat_expr_invalid_t::two_t*>(r->body()->at(2))->second(), std::string("\x37\x13\x00\x00", 4));
