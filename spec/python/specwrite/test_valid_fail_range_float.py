...
class TestValidFailRangeFloat(CommonSpec.Base):
    ...
    def test_check_bad_valid_no_io(self):
        r = ValidFailRangeFloat()
        r.foo = 0.5
        self.assert_check_valid_fail(r)

    def test_check_bad_valid_old_io(self):
        r = ValidFailRangeFloat.from_file(self.src_filename)
    ...
    def assert_check_valid_fail(self, r):
        ...
        self.assertEqual(str(cm.exception), "/seq/0: validation failed: not in range, max 0.375, but got 0.5")
