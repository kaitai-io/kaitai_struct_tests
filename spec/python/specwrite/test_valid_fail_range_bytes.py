...
class TestValidFailRangeBytes(CommonSpec.Base):
    ...
    def test_check_bad_valid_no_io(self):
        r = ValidFailRangeBytes()
        r.foo = b"\x50\x41"
        self.assert_check_valid_fail(r)

    def test_check_bad_valid_old_io(self):
        r = ValidFailRangeBytes.from_file(self.src_filename)
    ...
    def assert_check_valid_fail(self, r):
        ...
        self.assertEqual(str(cm.exception), "/seq/0: validation failed: not in range, max b'P1', but got b'PA'")
