...
class TestValidFailMinInt(CommonSpec.Base):
    ...
    def test_check_bad_valid_old_io(self):
        r = ValidFailMinInt.from_file(self.src_filename)
        with self.assertRaises(kaitaistruct.ValidationLessThanError):
            r._read()
        self.assert_check_valid_fail(r)

    def assert_check_valid_fail(self, r):
        with self.assertRaises(kaitaistruct.ValidationLessThanError) as cm:
            r._check()
        ...
        self.assertEqual(str(cm.exception), "/seq/0: validation failed: not in range, min 123, but got 80")
