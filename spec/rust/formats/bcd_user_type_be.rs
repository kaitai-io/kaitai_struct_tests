...
pub struct BcdUserTypeBe {
    pub ltr: Option<BcdUserTypeBe_LtrObj>,
    pub rtl: Option<BcdUserTypeBe_RtlObj>,
    pub leading_zero_ltr: Option<BcdUserTypeBe_LeadingZeroLtrObj>,
    pub raw_ltr: Vec<u8>,
    pub raw_rtl: Vec<u8>,
    pub raw_leading_zero_ltr: Vec<u8>,
}
impl<'r, 's: 'r> KStruct<'r, 's> for BcdUserTypeBe {
    type Root = Self;
    ...
    fn read<S: KStream>(
        ...
        self.ltr = Some(Self::read_into::<BytesReader, BcdUserTypeBe_LtrObj>(&BytesReader::new(_io.read_bytes(4 as usize)?), Some(self), _parent.push(self))?);
        self.rtl = Some(Self::read_into::<BytesReader, BcdUserTypeBe_RtlObj>(&BytesReader::new(_io.read_bytes(4 as usize)?), Some(self), _parent.push(self))?);
        self.leading_zero_ltr = Some(Self::read_into::<BytesReader, BcdUserTypeBe_LeadingZeroLtrObj>(&BytesReader::new(_io.read_bytes(4 as usize)?), Some(self), _parent.push(self))?);
        Ok(())
    }
}
impl<'r, 's: 'r> BcdUserTypeBe {
    pub fn from_file(path: &str) -> Self {
        let bytes = fs::read(path).unwrap();
        let reader = BytesReader::new(&bytes);
        let mut obj = BcdUserTypeBe::default();
...
impl<'r, 's: 'r> KStruct<'r, 's> for BcdUserTypeBe_LtrObj {
    type Root = BcdUserTypeBe;
    type ParentStack = (&'r BcdUserTypeBe, <BcdUserTypeBe as KStruct<'r, 's>>::ParentStack);
...
impl<'r, 's: 'r> BcdUserTypeBe_LtrObj {
    pub fn from_file(path: &str) -> Self {
        let bytes = fs::read(path).unwrap();
        let reader = BytesReader::new(&bytes);
        let mut obj = BcdUserTypeBe_LtrObj::default();
    ...
    fn as_int<S: KStream>(
        &mut self,
        _io: &'s S,
        _root: Option<&'r BcdUserTypeBe>,
        _parent: Option<TypedStack<(&'r BcdUserTypeBe, <BcdUserTypeBe as KStruct<'r, 's>>::ParentStack)>>
    ) -> KResult<&i32> {
    ...
    fn digit2<S: KStream>(
        ...
        self.digit2 = Some((self.b1 & 15) as i32);
        return Ok(self.digit2.as_ref().unwrap());
    }
    fn digit4<S: KStream>(
        &mut self,
        ...
        self.digit4 = Some((self.b2 & 15) as i32);
        return Ok(self.digit4.as_ref().unwrap());
    }
    fn digit3<S: KStream>(
        &mut self,
        ...
        self.digit3 = Some(((self.b2 & 240) >> 4) as i32);
        return Ok(self.digit3.as_ref().unwrap());
    }
    fn digit5<S: KStream>(
        &mut self,
        ...
        self.digit5 = Some(((self.b3 & 240) >> 4) as i32);
        return Ok(self.digit5.as_ref().unwrap());
    }
    fn digit8<S: KStream>(
        &mut self,
        ...
        self.digit8 = Some((self.b4 & 15) as i32);
        return Ok(self.digit8.as_ref().unwrap());
    }
    fn digit6<S: KStream>(
        &mut self,
        ...
        self.digit6 = Some((self.b3 & 15) as i32);
        return Ok(self.digit6.as_ref().unwrap());
    }
    fn as_str<S: KStream>(
        &mut self,
    ...
    fn digit1<S: KStream>(
        ...
        self.digit1 = Some(((self.b1 & 240) >> 4) as i32);
        return Ok(self.digit1.as_ref().unwrap());
    }
    fn digit7<S: KStream>(
        &mut self,
        ...
        self.digit7 = Some(((self.b4 & 240) >> 4) as i32);
        return Ok(self.digit7.as_ref().unwrap());
    }
}
...
impl<'r, 's: 'r> KStruct<'r, 's> for BcdUserTypeBe_RtlObj {
    type Root = BcdUserTypeBe;
    type ParentStack = (&'r BcdUserTypeBe, <BcdUserTypeBe as KStruct<'r, 's>>::ParentStack);
...
impl<'r, 's: 'r> BcdUserTypeBe_RtlObj {
    pub fn from_file(path: &str) -> Self {
        let bytes = fs::read(path).unwrap();
        let reader = BytesReader::new(&bytes);
        let mut obj = BcdUserTypeBe_RtlObj::default();
...
impl<'r, 's: 'r> KStruct<'r, 's> for BcdUserTypeBe_LeadingZeroLtrObj {
    type Root = BcdUserTypeBe;
    type ParentStack = (&'r BcdUserTypeBe, <BcdUserTypeBe as KStruct<'r, 's>>::ParentStack);
...
impl<'r, 's: 'r> BcdUserTypeBe_LeadingZeroLtrObj {
    pub fn from_file(path: &str) -> Self {
        let bytes = fs::read(path).unwrap();
        let reader = BytesReader::new(&bytes);
        let mut obj = BcdUserTypeBe_LeadingZeroLtrObj::default();
