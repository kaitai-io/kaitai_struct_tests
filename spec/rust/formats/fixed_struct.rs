...
pub struct FixedStruct {
    pub hdr: Option<FixedStruct_Header>,
}
impl<'r, 's: 'r> KStruct<'r, 's> for FixedStruct {
    type Root = Self;
...
impl<'r, 's: 'r> FixedStruct {
    pub fn from_file(path: &str) -> Self {
        let bytes = fs::read(path).unwrap();
        let reader = BytesReader::new(&bytes);
        let mut obj = FixedStruct::default();
    ...
    ) -> KResult<&FixedStruct_Header> {
        if self.hdr.is_some() {
            return Ok(self.hdr.as_ref().unwrap());
        }
        // pushPos(_io)
        // seek(_io, IntNum(0))
        // popPos(_io)
...
impl<'r, 's: 'r> KStruct<'r, 's> for FixedStruct_Header {
    type Root = FixedStruct;
    type ParentStack = (&'r FixedStruct, <FixedStruct as KStruct<'r, 's>>::ParentStack);
...
impl<'r, 's: 'r> FixedStruct_Header {
    pub fn from_file(path: &str) -> Self {
        let bytes = fs::read(path).unwrap();
        let reader = BytesReader::new(&bytes);
        let mut obj = FixedStruct_Header::default();
