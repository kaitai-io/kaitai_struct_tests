...
fn test_switch_manual_int_size_eos() {
    ...
    if let SwitchManualIntSizeEos_Chunk_ChunkBody_Body::SwitchManualIntSizeEos_Chunk_ChunkBody_ChunkMeta(s) =  r.chunks()[0].body().body() {
    ...
    if let SwitchManualIntSizeEos_Chunk_ChunkBody_Body::SwitchManualIntSizeEos_Chunk_ChunkBody_ChunkDir(s) =  r.chunks()[1].body().body() {
    ...
    if let SwitchManualIntSizeEos_Chunk_ChunkBody_Body::Bytes(s) = r.chunks()[2].body().body() {
    ...
    if let SwitchManualIntSizeEos_Chunk_ChunkBody_Body::Bytes(s) =  r.chunks()[3].body().body() {
        let raw : Vec<u8> = vec![];
        assert_eq!(raw, *s);
    } else {
        panic!("expected enum Bytes");
