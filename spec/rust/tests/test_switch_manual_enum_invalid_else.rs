...
fn test_switch_manual_enum_invalid_else() {
    ...
    let res: KResult<OptRc<SwitchManualEnumInvalidElse>> =
        SwitchManualEnumInvalidElse::read_into(&_io, None, None);
    ...
    assert_eq!(n, 255);
    assert_eq!(
        *Into::<OptRc<SwitchManualEnumInvalidElse_Opcode_Defval>>::into(
            &*r.opcodes()[0 as usize].body().as_ref().unwrap()
        )
        .value()
        .unwrap(),
        123
    ...
    assert_eq!(
        *Into::<OptRc<SwitchManualEnumInvalidElse_Opcode_Defval>>::into(
            &*r.opcodes()[1 as usize].body().as_ref().unwrap()
        )
        .value()
        .unwrap(),
        123
