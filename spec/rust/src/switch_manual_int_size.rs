...
impl KaitaiStruct for SwitchManualIntSize {
    ...
    fn read<S: KaitaiStream>(&mut self,
        ...
        $i = 0;
        while (!stream->isEof()) {
            self.chunks[] = new switch_manual_int_size::chunk(stream, $this, _root);
            $i++;
        }
    ...
    fn read<S: KaitaiStream>(&mut self,
        ...
        switch ($this->code()) {
            case 17:
                ...
                self.body = new switch_manual_int_size::chunk::chunk_meta($io, $this, _root);
                break;
            case 34:
                self._raw_body = stream->readBytes($this->size());
                $io = new &mut S(self._raw_body);
                self.body = new switch_manual_int_size::chunk::chunk_dir($io, $this, _root);
                break;
            default:
