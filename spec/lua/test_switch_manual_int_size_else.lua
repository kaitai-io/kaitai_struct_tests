...
TestSwitchManualIntSizeElse = {}

function TestSwitchManualIntSizeElse:test_switch_manual_int_size_else()
    local r = SwitchManualIntSizeElse:from_file("src/switch_tlv.bin")
    ...
    luaunit.assertEquals(r.chunks[3].body.rest, "\x10\x20\x30\x40\x50\x60\x70\x80")

    luaunit.assertEquals(r.chunks[4].code, 0xff)
    luaunit.assertEquals(r.chunks[4].body.rest, "")
end
