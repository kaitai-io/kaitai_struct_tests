...
test "SwitchManualIntSizeElse":
  ...
  check((SwitchManualIntSizeElse_Chunk_ChunkMeta(r.chunks[0].body)).title == string("Stuff"))
  check((SwitchManualIntSizeElse_Chunk_ChunkMeta(r.chunks[0].body)).author == string("Me"))
  check(r.chunks[1].code == uint8(34))
  check((SwitchManualIntSizeElse_Chunk_ChunkDir(r.chunks[1].body)).entries == @[string("AAAA"), "BBBB", "CCCC"])
