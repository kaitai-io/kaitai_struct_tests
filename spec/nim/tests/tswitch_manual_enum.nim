...
var
  stream = newFileStream("test_out/nim/junitreports/switch_manual_enum.xml", fmWrite)
  outputFormatter = newJUnitOutputFormatter(stream)
addOutputFormatter(outputFormatter)

suite "Kaitai Struct Compiler Test Suite":
  test "Nim: SwitchManualEnum":
    let r = SwitchManualEnum.fromFile("src/switch_opcodes.bin")

    check(len(r.opcodes) == int(4))
    check(r.opcodes[0].code == CodeEnum())
    check(r.opcodes[0].body.value == string("foobar"))
    check(r.opcodes[1].code == CodeEnum())
    check(r.opcodes[1].body.value == uint8(66))
    check(r.opcodes[2].code == CodeEnum())
    check(r.opcodes[2].body.value == uint8(55))
    check(r.opcodes[3].code == CodeEnum())
