...
import io.kaitai.struct.testtranslator.{Main, TestEquals, TestSpec, ExpectedException}
...
class CSG(spec: TestSpec, provider: ClassTypeProvider) extends BaseGenerator(spec) {
  ...
  override def runParseExpectError(expException: ExpectedException): Unit = {
    runParseCommon1()
    out.puts(s"BOOST_CHECK_EQUAL(error, ${CCompiler.ksErrorName(expException.exception)});")
  }
  ...
  override def nullAssert(actual: Ast.expr): Unit = {
    val actualType = translator.detectType(actual)
    if (isPointerLike(actualType)) {
      val actStr = translateAct(actual)
      out.puts(s"BOOST_CHECK($actStr == 0);")
      return
    }

    actual match {
      case Ast.expr.Attribute(x, Ast.identifier(attrName)) =>
        val nullCheckStr = fixup("!" + translator.anyField(x, s"_is_valid_$attrName"))
  ...
  def translateAct(x: Ast.expr): String = fixup(translator.translate(x))

  private def isPointerLike(dataType: DataType): Boolean = dataType match {
    case _: UserType | _: StrType | _: BytesType | _: ArrayType |
         KaitaiStreamType | OwnedKaitaiStreamType | KaitaiStructType |
         CalcKaitaiStructType(_) | AnyType =>
      true
    case _ =>
      false
  }
